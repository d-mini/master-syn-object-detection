...
from helpers import enable_physics, clean_up, jitter_scale, load_distractors, load_materials, random_color_material, create_primitive_objects, load_targets, set_basic_light
...
parser.add_argument("--type-level", type=int, default=0, choices=[0, 1], help="Type level: 0 (primitives with random colors), 1 (only real objects)")
...
OUTPUT_DIR = f"../output/scenario{args.scenario}/dr_dis{args.type_level}/inst{args.inst_per_class}_seed{args.seed}"
...
primitive_objs = create_primitive_objects()
distractor_objs = load_distractors()

# ----- Load Materials -----
materials = bproc.material.collect_all()
cc_mats = load_materials()
...
original_objs = set(target_objs + primitive_objs + distractor_objs + background)
...
while min(class_counts.get(cid, 0) for cid in target_class_ids) < instances_per_class or min(object_counts.get(oid, 0) for oid in object_ids) < instances_per_object or neg_count < int(scene_id * 4 * neg_ratio):
    ...
    sampled_dist_objs = []
    n_min = cfg['sampling']['min_distractor_objs']
    n_max = cfg['sampling']['max_distractor_objs']
    n_total = np.random.randint(n_min, n_max + 1)

    if args.type_level == "T0":
        prims = np.random.choice(primitive_objs, size=n_total, replace=False)
        sampled_dist_objs.extend(prims)
    elif args.type_level == "T1":
        distractors = np.random.choice(distractor_objs, size=n_total, replace=False)
        sampled_dist_objs.extend(distractors)

    
    for obj in primitive_objs:
        obj.hide(True)
    ...
    for obj in sampled_target_objs:
        enable_physics(obj)
        obj.hide(False)
    for obj in sampled_dist_objs:
        if args.type_level == "T0":
            jitter_scale(obj, value=1, per_axis=True, reset=True)
            random_color_material(obj)
        enable_physics(obj)
        obj.hide(False)

    bproc.object.sample_poses_on_surface(sampled_target_objs+sampled_dist_objs, ground, sample_pose, min_distance=cfg['sampling']['min_distance'], max_distance=cfg['sampling']['max_distance'])
    bproc.object.simulate_physics_and_fix_final_poses(min_simulation_time=3, max_simulation_time=10, check_object_interval=1)

    bvh_tree = bproc.object.create_bvh_tree_multi_objects(sampled_target_objs+sampled_dist_objs+background)
    ...
    for radius_set in radius_sets:
        ...
        while poses < poses_per_set:
            poi = bproc.object.compute_poi(sampled_target_objs+sampled_dist_objs)
    ...
    # Render the scene
    bproc.renderer.set_world_background(cfg['rendering']['default_background_color'], strength=1)
