...
from helpers import enable_physics, clean_up, load_targets, set_basic_light, load_materials, random_material
...
OUTPUT_DIR = f"../output/scenario{args.scenario}/dr_hin0/inst{args.inst_per_class}_seed{args.seed}"
...
while min(class_counts.get(cid, 0) for cid in target_class_ids) < instances_per_class or min(object_counts.get(oid, 0) for oid in object_ids) < instances_per_object or neg_count < int(scene_id * 4 * neg_ratio):
    ...
    random_material(ground, cc_mats)
