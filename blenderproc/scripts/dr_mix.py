...
from helpers import enable_physics, clean_up, load_targets, load_distractors, load_materials, random_material


# ----- Argument and config parsing ------

parser = argparse.ArgumentParser()
parser.add_argument("--scenario", type=int, required=True, help="Scenario number (1/2)", choices=[1, 2])
...
OUTPUT_DIR = f"../output/scenario{args.scenario}/dr_mix/{args.scene}/inst{args.inst_per_class}_seed{args.seed}"
...
original_objs = set(target_objs + distractor_objs + background)
...
while min(class_counts.get(cid, 0) for cid in target_class_ids) < instances_per_class or min(object_counts.get(oid, 0) for oid in object_ids) < instances_per_object:
    ...
    bvh_tree = bproc.object.create_bvh_tree_multi_objects(sampled_target_objs+sampled_dist_objs)
...
if neg_count < neg_images_needed:
    ...
    bproc.object.sample_poses_on_surface(sampled_dist_objs, ground, sample_pose, min_distance=cfg['sampling']['min_distance'], max_distance=cfg['sampling']['max_distance'])
    bproc.object.simulate_physics_and_fix_final_poses(min_simulation_time=3, max_simulation_time=10, check_object_interval=1)

    poi = bproc.object.compute_poi(sampled_dist_objs)

    print(f"\nGenerating additional negative image {neg_count + 1} of {neg_images_needed}")
    while neg_count < neg_images_needed:
        location = bproc.sampler.shell(center=poi,
                                        radius_min=radius_min,
                                        radius_max=radius_max,
                                        elevation_min=10,
